...
public class ListarCervejasActivity extends SherlockListActivity {
	...
	private static final String TAG = ListarCervejasActivity.class
			.getSimpleName();
	...
	private void parsearJSON(String json) {
		...
		try {
			...
			for (int i = 0; i < qtdDeCervejas; i++) {
				...
				cerva.setCervejaria(jObjCerva.getJSONObject("brewery")
						.getString("name"));
	...
	protected void listarCervejas(ArrayList<Cerveja> cervejas) {
		...
		Log.i(TAG, "threadID em listarCervejas: "
				+ Thread.currentThread().getId());

		this.cervejas = cervejas;

		if (cervejas.size() > 0) {
			ListaCervejasAdapter adapter = new ListaCervejasAdapter(this,
					cervejas);
